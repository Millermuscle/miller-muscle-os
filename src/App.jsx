    ...
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div style={{width:'100%',maxWidth:400}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:24}}>
          <div style={{fontSize:16,fontWeight:800,color:C.text,fontFamily:"'Barlow Condensed',sans-serif",marginBottom:20}}>{mode==='login'?'COACH / CLIENT LOGIN':'CREATE ACCOUNT'}</div>
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            ...
            {error&&<div style={{fontSize:12,color:error.includes('Check')?C.green:C.red,padding:'8px 12px',background:error.includes('Check')?'#22c55e15':'#ef444415',borderRadius:8}}>{error}</div>}
            ...
            <button onClick={()=>setMode(mode==='login'?'signup':'login')}
              style={{padding:'10px 0',background:'transparent',border:`1px solid ${C.border}`,borderRadius:10,color:C.textMuted,fontSize:12,cursor:'pointer'}}>
...
const TA=({label,value,onChange,placeholder,rows=3})=>(
  <div>{label&&<Lbl>{label}</Lbl>}
    <textarea value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} rows={rows}
      style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,padding:"11px 14px",color:C.text,fontSize:13,outline:"none",resize:"none",lineHeight:1.6,boxSizing:"border-box"}}/>
  </div>
);
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
      {options.map(opt=>{
        ...
        return <button key={opt} onClick={()=>multi?onChange(active?(value||[]).filter(x=>x!==opt):[...(value||[]),opt]):onChange(opt)}
          style={{padding:"8px 13px",background:active?col:C.card,border:`1px solid ${active?col:C.border}`,borderRadius:8,color:active?"#fff":C.textMuted,fontSize:12,fontWeight:active?700:400,cursor:"pointer"}}>{opt}</button>;
      })}
    </div>
  </div>;
};
...
const Done=({title,sub,onBack})=>(
  <div style={{textAlign:"center",padding:"60px 20px",display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
    ...
    <button onClick={onBack} style={{marginTop:8,padding:"11px 22px",background:"transparent",border:`1px solid ${C.border}`,borderRadius:10,color:C.textMuted,fontSize:12,cursor:"pointer"}}>EDIT</button>
  </div>
);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {[
        ...
        {id:"enhanced",icon:"◉",title:"Enhanced Athlete",sub:"Competitor form + compound tracking",color:C.purple},
      ].map(opt=>(
        <button key={opt.id} onClick={()=>onSelect(opt.id)}
          style={{display:"flex",alignItems:"center",gap:14,padding:"16px",background:C.card,border:`1px solid ${C.border}`,borderRadius:14,cursor:"pointer",textAlign:"left"}}>
...
function GeneralCheckIn({onDone}){
  ...
  const pages=[
    ...
    <div key="d2" style={{display:"flex",flexDirection:"column",gap:16}}>
      <ST>Diet</ST>
    ...
    <div key="e2" style={{display:"flex",flexDirection:"column",gap:16}}>
      <ST>Recovery</ST>
    ...
    <div key="f" style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <TA label="Questions / Concerns" value={d.questions} onChange={v=>s("questions",v)} placeholder="I'm here for you so lets talk."/>
    </div>
  ];
  return (
    <div>
...
function CompetitorCheckIn({onDone,enhanced=false}){
  ...
  const pages=[
    <div key="p1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Progress Photos</ST><Pills label="Did you post progress pics today?" options={["Yes","No"]} value={d.photos} onChange={v=>s("photos",v)}/><TI label="Current Weight (lbs)" value={d.weight} onChange={v=>s("weight",v)} placeholder="184.6" type="number"/></div>,
    <div key="t1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Training</ST><Pills label="How was training this week?" options={["Smashed it","Solid, maybe had more","Fair, not really feeling it","Garbage"]} value={d.training} onChange={v=>s("training",v)}/><TA label="IF off why?" value={d.trainingOff} onChange={v=>s("trainingOff",v)} placeholder="What was off?" rows={2}/><Scale label="Rank gym performance" value={d.gymPerf} onChange={v=>s("gymPerf",v)} lo="Struggle bus" hi="Nailed it"/><Pills label="Strength trend" options={["Stable","Slight drop","Significant drop"]} value={d.strengthTrend} onChange={v=>s("strengthTrend",v)}/></div>,
    <div key="c1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Cardio</ST><Pills label="Cardio / steps completed?" options={["Yes","Most","Missed","I was bad"]} value={d.cardio} onChange={v=>s("cardio",v)}/><TA label="IF not what happened?" value={d.cardioWhat} onChange={v=>s("cardioWhat",v)} placeholder="What got in the way?" rows={2}/><Pills label="Cardio feel harder?" options={["No","Slightly","Much harder"]} value={d.cardioHarder} onChange={v=>s("cardioHarder",v)}/><Pills label="Leg fatigue affecting cardio?" options={["Yes","Slight","No"]} value={d.legFatigue} onChange={v=>s("legFatigue",v)}/></div>,
    <div key="n1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Nutrition</ST><Pills label="Nutrition compliance" options={["100%","Minor issue","Off plan"]} value={d.nutrition} onChange={v=>s("nutrition",v)}/>{d.nutrition&&d.nutrition!=="100%"&&<TA label="IF off what happened?" value={d.nutritionOff} onChange={v=>s("nutritionOff",v)} placeholder="No judgment"/>}</div>,
    <div key="r1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Recovery</ST><Pills label="Avg hours sleep" options={["7+","5-6","<5"]} value={d.avgSleep} onChange={v=>s("avgSleep",v)}/><Scale label="Sleep quality" value={d.sleepQuality} onChange={v=>s("sleepQuality",v)} lo="Insomnia" hi="Like a baby"/><Scale label="Stress level" value={d.stress} onChange={v=>s("stress",v)} lo="Calm" hi="Freaking out" inv/></div>,
    <div key="m1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Mindset</ST><Pills label="Motivation to train" options={["High","Normal","Low"]} value={d.motivation} onChange={v=>s("motivation",v)}/><Pills label="Enjoying the program?" options={["LOVE IT","Its good","Ready for change"]} value={d.enjoyment} onChange={v=>s("enjoyment",v)}/></div>,
    <div key="di1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Digestion</ST><Pills label="How is digestion?" options={["Normal","Bloating","Constipation","Loose stool"]} value={d.digestion} onChange={v=>s("digestion",v)}/></div>,
    <div key="po1" style={{display:"flex",flexDirection:"column",gap:16}}><ST color={C.gold}>Posing Practice</ST><Pills label="Did you practice posing?" options={["Yes","No","Need help"]} value={d.posing} onChange={v=>s("posing",v)}/></div>,
    <div key="w1" style={{display:"flex",flexDirection:"column",gap:16}}><ST>Wrap Up</ST><TA label="Hardest part of the week" value={d.hardest} onChange={v=>s("hardest",v)} placeholder="Biggest challenge?"/><TA label="Anything I need to know?" value={d.needToKnow} onChange={v=>s("needToKnow",v)} placeholder="Text me if its important!"/><TA label="Questions / Concerns?" value={d.questions} onChange={v=>s("questions",v)} placeholder="I am here for you so lets talk."/></div>,
    enhanced?(
      <div key="enh1" style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <TI label="Cycle week #" value={d.cycleWeek} onChange={v=>s("cycleWeek",v)} placeholder="8" type="number"/>
        ...
        <div>
          ...
          <input type="date" value={d.lastLabs} onChange={e=>s("lastLabs",e.target.value)}
            style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,padding:"11px 14px",color:C.text,fontSize:14,outline:"none",boxSizing:"border-box",colorScheme:"dark"}}/>
